...
export default function ChefEtablissementDashboard() {
  ...
  const executerActionAnneeScolaire = async () => {
    ...
    if (actionType === 'ouvrir') {
      ...
      const anneeIdFermee = anneeActive.id;

      // [NOUVEAU] Archive automatiquement toutes les fiches de l'année qui
      // se ferme dans la bibliothèque pédagogique — avant, "fermer l'année"
      // ne faisait QUE désactiver l'année, laissant les fiches des
      // enseignants orphelines (ni visibles pour eux, ni dans la
      // bibliothèque). Même méthode que le rattrapage ponctuel déjà fait
      // en SQL, mais désormais automatique à chaque fermeture.
      const { data: affiliationsEtab } = await supabase
        .from('affiliations_etablissement').select('id').eq('etablissement_id', etablissementId);
      const idsAffiliations = (affiliationsEtab || []).map(a => a.id);

      if (idsAffiliations.length > 0) {
        const { data: programmesAFermer } = await supabase
          .from('programmes_annuels').select('id, proprietaire_user_id')
          .in('affiliation_id', idsAffiliations)
          .eq('annee_scolaire_id', anneeIdFermee);
        const idsProgrammes = (programmesAFermer || []).map(p => p.id);
        const proprietaireParProgramme = {};
        (programmesAFermer || []).forEach(p => { proprietaireParProgramme[p.id] = p.proprietaire_user_id; });

        if (idsProgrammes.length > 0) {
          const { data: cyclesAFermer } = await supabase
            .from('cycles').select('id, titre, programme_annuel_id').in('programme_annuel_id', idsProgrammes);
          const idsCycles = (cyclesAFermer || []).map(c => c.id);
          const cycleParId = {};
          (cyclesAFermer || []).forEach(c => { cycleParId[c.id] = c; });

          const { data: leconsAFermer } = idsCycles.length > 0
            ? await supabase.from('lecons').select('id, titre, cycle_id').in('cycle_id', idsCycles)
            : { data: [] };
          const idsLecons = (leconsAFermer || []).map(l => l.id);
          const leconParId = {};
          (leconsAFermer || []).forEach(l => { leconParId[l.id] = l; });

          const { data: seancesAFermer } = idsLecons.length > 0
            ? await supabase.from('seances').select('id, lecon_id, classe_id, contenu_json, motif_report').in('lecon_id', idsLecons)
            : { data: [] };

          if ((seancesAFermer || []).length > 0) {
            const idsClasses = [...new Set(seancesAFermer.map(s => s.classe_id).filter(Boolean))];
            const { data: classesData } = idsClasses.length > 0
              ? await supabase.from('classes').select('id, nom').in('id', idsClasses)
              : { data: [] };
            const classeParId = {};
            (classesData || []).forEach(c => { classeParId[c.id] = c.nom; });

            const idsSeances = seancesAFermer.map(s => s.id);
            const { data: dejaArchivees } = await supabase
              .from('bibliotheque_etablissement').select('seance_origine_id').in('seance_origine_id', idsSeances);
            const idsDejaArchivees = new Set((dejaArchivees || []).map(a => a.seance_origine_id));

            const lignesArchive = seancesAFermer
              .filter(s => !idsDejaArchivees.has(s.id))
              .map(s => {
                const lecon = leconParId[s.lecon_id];
                const cycle = lecon ? cycleParId[lecon.cycle_id] : null;
                return {
                  etablissement_id: etablissementId,
                  annee_scolaire_id: anneeIdFermee,
                  seance_origine_id: s.id,
                  auteur_user_id: cycle ? proprietaireParProgramme[cycle.programme_annuel_id] : null,
                  titre: s.contenu_json?.titre || 'Séance',
                  contenu_snapshot_json: {
                    classe: classeParId[s.classe_id] || '',
                    cycle: cycle?.titre || '',
                    lecon: lecon?.titre || '',
                    seance: s.contenu_json,
                    motif_report: s.motif_report,
                  },
                };
              });

            if (lignesArchive.length > 0) {
              await supabase.from('bibliotheque_etablissement').insert(lignesArchive);
            }
          }
        }
      }
      ...
      setAnneeActive(null);
      showToast("🔒 Année scolaire clôturée. Toutes les fiches ont été archivées dans la bibliothèque pédagogique.");

      await envoyerNotificationATousLesMembres(
        etablissementId, 'ANNEE_FERMEE',
        `🔒 L'année scolaire "${intituleFerme}" a été clôturée. Vos fiches sont désormais dans la bibliothèque pédagogique.`,
        'profil_ecole'
      );
    }
