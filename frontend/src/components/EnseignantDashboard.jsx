...
export default function EnseignantDashboard() {
  ...
  const chargerTout = async () => {
    ...
    const affiliationsFormatees = (affiliationsData || []).map(a => ({
      ...
      // [NOUVEAU] Déduplique par nom — protège l'affichage même si d'anciens
      // doublons d'attribution traînent encore en base pour cet enseignant
      // (une même classe ne doit jamais apparaître deux fois dans sa liste).
      classes: [...new Set(
        (attributions || [])
    ...
    // propre à chaque classe (même logique de liaison par titre que ci-dessus) ---
      ...
      for (const classeCible of ciblesSeance) {
        ...
        if (modalAssistant.creeeCommeReport) {
          ...
          // [NOUVEAU] Le censeur doit voir un report en temps réel, comme
          // un imprévu du jour — pas seulement à sa prochaine connexion.
          // Le canal temps réel déjà en place côté Censeur rafraîchit
          // automatiquement son écran dès que cette notification arrive.
          await notifierCenseurNouvelleFiche(classeCible, `↩️ Séance reportée (${classeCible}) — motif : ${modalAssistant.motifReportCreation.trim()}`);
        }
  ...
  const soumettreAuCenseur = async (type, cycleId, leconId = null, seanceId = null) => {
    ...
    if (type === 'seance' && seanceId) {
      // [CORRIGÉ] Un job pg_cron (voir le fichier SQL fourni) fait
      // maintenant automatiquement passer une séance "PROGRAMMEE" en
      // "ENVOYEE" dès que sa date arrive — ce mécanisme, absent avant,
      // laissait ces séances invisibles pour toujours chez le censeur.
      let dateSeance = null;
      let leconCible = null;
  ...
  const soumettreDemandeAffiliation = async (e) => {
    ...
    // [NOUVEAU] Le censeur et le chef sont notifiés immédiatement (cloche),
    // au lieu de découvrir la demande seulement en rouvrant leur onglet —
    // avant, personne n'était notifié à ce moment-là.
    const { data: destinatairesActifs } = await supabase
      .from('affiliations_etablissement')
      .select('user_id, role')
      .eq('etablissement_id', etablissementCible.id)
      .in('role', ['CENSEUR', 'CHEF'])
      .eq('statut', 'ACTIVE');
    for (const dest of (destinatairesActifs || [])) {
      const lienPourRole = dest.role === 'CENSEUR' ? 'professeurs' : 'censeurs';
      await envoyerNotification(
        dest.user_id, 'DEMANDE_AFFILIATION_RECUE',
        `👥 Nouvelle demande d'affiliation d'un enseignant`,
        lienPourRole, etablissementCible.id
      );
    }
...
const styles = {
  ...
  carteClasseItem: { backgroundColor: '#ffffff', padding: '22px', borderRadius: '18px', border: '1px solid #edf1f7', boxShadow: '0 2px 8px rgba(15,23,42,0.04), 0 12px 32px -16px rgba(15,23,42,0.06)', boxSizing: 'border-box' },
